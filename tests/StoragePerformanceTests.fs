...
type StoragePerformanceTests() =
    ...
    let createTestTask (id: int) =
        { TaskId = $"perf-test-{id:D6}"
          ...
          Status = if id % 4 = 0 then TaskStatus.Completed else TaskStatus.Pending
          AssignedAgent = if id % 3 = 0 then Some $"agent-{id % 5}" else None
          Priority = match id % 4 with
                     | 0 -> TaskPriority.Critical
                     | 1 -> TaskPriority.High 
                     | 2 -> TaskPriority.Medium
                     | _ -> TaskPriority.Low
          EstimatedDuration = if id % 2 = 0 then Some(TimeSpan.FromMinutes(float (id % 120))) else None
          ActualDuration = if id % 6 = 0 then Some(TimeSpan.FromMinutes(float (id % 60))) else None
          RequiredResources = 
            match id % 3 with
            | 0 -> ["cpu"; "memory"]
            | 1 -> ["disk"; "network"; "gpu"]
    ...
    member _.``Compare bulk insert performance: 6-table vs 3-table design``() =
        async {
            ...
            // 3テーブル設計のパフォーマンステスト
            let tempDbPath3 = Path.Combine(Path.GetTempPath(), $"perf_3table_{Guid.NewGuid()}.db")
            let connectionString3 = $"Data Source={tempDbPath3};"
            
            let stopwatch3 = Stopwatch.StartNew()
            try
                use threeTableStorage = TaskStorageFactory.CreateTaskStorage(connectionString3, ThreeTableDesign)
                ...
                // 6テーブル設計のパフォーマンステスト
                let tempDbPath6 = Path.Combine(Path.GetTempPath(), $"perf_6table_{Guid.NewGuid()}.db")
                ...
                try
                    ...
                    Console.WriteLine($"  3-Table Design: {elapsed3} ms ({float taskCount / float elapsed3 * 1000.0:F2} tasks/sec)")
                    Console.WriteLine($"  6-Table Design: {elapsed6} ms ({float taskCount / float elapsed6 * 1000.0:F2} tasks/sec)")
                    Console.WriteLine($"  Performance Ratio: {float elapsed6 / float elapsed3:F2}x (higher = 3-table is faster)")
                    ...
                    Assert.IsTrue(float elapsed3 / float elapsed6 < 2.0, "3-table design should not be more than 2x slower")
                    
                finally
                    if File.Exists(tempDbPath6) then File.Delete(tempDbPath6)
            finally
                if File.Exists(tempDbPath3) then File.Delete(tempDbPath3)
    ...
    member _.``Compare query performance: GetExecutableTasks``() =
        async {
            ...
            // 3テーブル設計のクエリパフォーマンス
            let tempDbPath3 = Path.Combine(Path.GetTempPath(), $"query_perf_3table_{Guid.NewGuid()}.db")
            let connectionString3 = $"Data Source={tempDbPath3};"
            ...
            try
                ...
                // 依存関係追加（一部のタスクのみ）
                for i in [1..50] do
                    let! _ = threeTableStorage.SaveTaskDependency($"perf-test-{i:D6}", $"perf-test-{i+100:D6}", "hard")
                    ()
                ...
                // 6テーブル設計のクエリパフォーマンス
                let tempDbPath6 = Path.Combine(Path.GetTempPath(), $"query_perf_6table_{Guid.NewGuid()}.db")
                let connectionString6 = $"Data Source={tempDbPath6};"
                ...
                try
                    ...
                    // 依存関係追加
                    for i in [1..50] do
                        let! _ = sixTableStorage.SaveTaskDependency($"perf-test-{i:D6}", $"perf-test-{i+100:D6}", "hard")
                        ()
                    ...
                    | Result.Ok(tasks3), Result.Ok(tasks6) ->
                        ...
                        Assert.IsTrue(diff <= tolerance, $"Task count difference ({diff}) should be within tolerance ({tolerance}). 3-table: {tasks3.Length}, 6-table: {tasks6.Length}")
    ...
    member _.``Compare memory usage during operations``() =
        async {
            ...
            // 3テーブル設計のメモリ使用量測定
            let tempDbPath3 = Path.Combine(Path.GetTempPath(), $"mem_3table_{Guid.NewGuid()}.db")
            let connectionString3 = $"Data Source={tempDbPath3};"
            ...
            try
                ...
                // 6テーブル設計のメモリ使用量測定
                let tempDbPath6 = Path.Combine(Path.GetTempPath(), $"mem_6table_{Guid.NewGuid()}.db")
                let connectionString6 = $"Data Source={tempDbPath6};"
