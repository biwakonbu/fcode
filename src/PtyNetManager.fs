...
type PtyNetManager() =
            ...
            try
                ...
                proc.OutputDataReceived.Add(fun e ->
                    if not (String.IsNullOrEmpty(e.Data)) then
                        lock outputBufferLock (fun () ->
                            session.OutputBuffer.AppendLine(e.Data) |> ignore
                        )
                )
                ...
                proc.ErrorDataReceived.Add(fun e ->
                    if not (String.IsNullOrEmpty(e.Data)) then
                        lock outputBufferLock (fun () ->
                            session.OutputBuffer.AppendLine("[ERR] " + e.Data) |> ignore
                        )
                )
    ...
    member this.StartOutputReading() : Task<unit> =
        task {
            ...
            | _ -> 
                logWarning "出力読み取り開始" "アクティブなセッションが存在しません"
        }
    ...
    member this.CloseSession() : unit =
        ...
        | Some session ->
            ...
            with
            | ex -> logError "セッション終了エラー" (ex.Message)
