...
type NextSprintPlannerManager() =
    ...
    /// 次スプリント計画生成
    member this.PlanNextSprint(completion: CompletionAssessment, quality: QualityEvaluationSummary, resources: string list) : NextSprintPlan =
        // スプリントID生成
    ...
    /// 優先タスク決定
    member private this.DeterminePriorityTasks(completion: CompletionAssessment, quality: QualityEvaluationSummary) : string list =
        let mutable tasks = []
    ...
    /// 依存関係分析
    member private this.AnalyzeDependencies(completion: CompletionAssessment, quality: QualityEvaluationSummary) : string list =
    ...
    /// リスク軽減策計画
    member private this.PlanRiskMitigation(completion: CompletionAssessment, quality: QualityEvaluationSummary, resources: string list) : string list =
