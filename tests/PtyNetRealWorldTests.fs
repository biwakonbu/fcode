            ...
            | Some manager ->
                ...
                for (cmd, args, expectedContent) in basicCommands do
                    ...
                    | Result.Ok _session ->
                        ...
                        Assert.That(output.Contains(expectedContent), Is.True, 
                    ...
                    | Result.Error error ->
                        Assert.Fail(cmd + "コマンドセッション作成に失敗: " + error)
    ...
    member _.MultipleSessionsConcurrent_Isolation() =
        async {
            ...
            | Some manager1 ->
                ...
                // すべてのセッションが成功することを確認
                Assert.That(session1Result, Is.TypeOf<Result<PtySession, string>>().And.Property("IsOk").True, "セッション1作成失敗")
                Assert.That(session2Result, Is.TypeOf<Result<PtySession, string>>().And.Property("IsOk").True, "セッション2作成失敗")
                Assert.That(session3Result, Is.TypeOf<Result<PtySession, string>>().And.Property("IsOk").True, "セッション3作成失敗")
                ...
                // クロス汚染がないことを確認
                Assert.That(output1.Contains("Session-2") || output1.Contains("Session-3"), Is.False, "セッション1に他の出力が混入しています")
                Assert.That(output2.Contains("Session-1") || output2.Contains("Session-3"), Is.False, "セッション2に他の出力が混入しています")
                Assert.That(output3.Contains("Session-1") || output3.Contains("Session-2"), Is.False, "セッション3に他の出力が混入しています")
                
                logInfo "複数セッションテスト結果" ("session1=" + output1.Length.ToString() + ", session2=" + output2.Length.ToString() + ", session3=" + output3.Length.ToString())
        ...
        async {
            ...
            | Some manager ->
                ...
                | Result.Ok _session ->
                    ...
                    Assert.That(afterDelayOutput.Length, Is.EqualTo(finalOutput.Length), 
                        "セッション終了後に出力が増加しています（適切に終了していない可能性）")
                        
                | Result.Error error ->
                    Assert.Fail("長時間実行コマンドセッション作成に失敗: " + error)
            ...
            | Some manager ->
                ...
                for maliciousInput in maliciousInputs do
                    ...
                    | Result.Ok _session ->
                        ...
                        Assert.That(output.Contains(maliciousInput), Is.True, 
                        ...
                        logInfo "セキュリティテスト結果" ("入力: " + maliciousInput + " 出力: " + output.Substring(0, min 100 output.Length))
                        
                        // 出力に含まれている場合は、実行ではなくecho出力であることを確認
                        if output.Contains("/etc/passwd") then
                            // これはechoコマンドの引数として処理されたもので、実際のファイル内容でない場合は安全
                            Assert.That(output.Contains("root:x:"), Is.False, "実際のpasswdファイルが読み込まれた可能性: " + maliciousInput)
