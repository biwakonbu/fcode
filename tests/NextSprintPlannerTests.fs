...
let ``NextSprintPlannerManager - 基本的な次スプリント計画テスト`` () =
    ...
    // Act: 次スプリント計画生成
    let sprintPlan = manager.PlanNextSprint(completionAssessment, qualitySummary, availableResources)
...
let ``NextSprintPlannerManager - リソース制約時の計画テスト`` () =
    ...
    // Act: リソース制約時の計画
    let sprintPlan = manager.PlanNextSprint(completionAssessment, qualitySummary, limitedResources)
