...
type CompletionAssessmentManager() =
    ...
    member this.EvaluateCompletion(tasks: TaskInfo list, acceptanceCriteria: string list) : CompletionAssessment =
        ...
        let inProgressTasks = tasks |> List.filter (fun t -> t.Status = TaskStatus.InProgress)
        let blockedTasks = tasks |> List.filter (fun t -> t.Status = TaskStatus.Failed || t.Status = TaskStatus.Cancelled)
        
        // 完了率計算
        let totalTasks = tasks.Length
        let completionRate = 
            if totalTasks = 0 then 0.0
            else float completedTasks.Length / float totalTasks
        
        // 品質スコア計算（完了タスクの平均品質）
        let qualityScore = 
            if completedTasks.IsEmpty then 0.0
            else 
                let totalQuality = completedTasks |> List.sumBy (fun t -> priorityToScore t.Priority)
