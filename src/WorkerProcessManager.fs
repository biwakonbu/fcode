    ...
    let startWorkerProcess (paneId: string) (workingDir: string) (textView: TextView) =
        try
            ...
            if success then
                ...
                // Worker IO の作成
                let workerIO = {
                    StandardInput = fun input ->
                        try
                            // IPC経由で入力を送信
                            let sendInputTask = task {
                                try
                                    ...
                                    | Some _ -> 
                                        ...
                                        updateUI("")
                                    | None ->
                                        logError "WorkerManager" $"Failed to send input via IPC for pane: {paneId}"
                                with ex ->
                                    logException "WorkerManager" $"Error sending input via IPC for pane: {paneId}" ex
                            }
                ...
                // 初期メッセージを表示
                outputBuffer.AppendLine($"[DEBUG] Worker Process セッション開始完了 - ペイン: {paneId}") |> ignore
    ...
    let stopWorkerProcess (paneId: string) =
        try
            ...
            | Some workerInfo ->
                ...
                if success then
                    let updatedWorker = { workerInfo with Status = Stopping; IO = None }
