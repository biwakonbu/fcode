...
type SessionIsolationTests() =
    ...
    member _.BasicWorkspaceCreationTest() =
        let config = { WorkingDirectoryManager.defaultConfig with
        ...
        | Error e ->
            Assert.Fail(sprintf "ワークスペース作成失敗: %s" e)
    ...
    member _.BasicEnvironmentIsolationTest() =
        ...
        | Error e ->
            Assert.Fail(sprintf "環境作成失敗: %s" e)

    /// 基本的なファイルロックテスト
    [<Test>]
    member _.BasicFileLockTest() =
        let config = { FileLockManager.defaultConfig with
        ...
        | result ->
            Assert.Fail(sprintf "ロック取得失敗: %A" result)

    /// 基本的なセッション状態管理テスト
    [<Test>]
    member _.BasicSessionStateTest() =
        let config = { SessionStateManager.defaultConfig with
        ...
        | Error e ->
            Assert.Fail(sprintf "セッション作成失敗: %s" e)