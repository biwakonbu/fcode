...
**最終更新**: 2025-06-25  
**総実装ライン数**: 1200行 (src/), 500行 (tests/)  
**テストカバレッジ**: 50テストケース / 100%パス  
**🎯 Phase 1完了**: 単一ペイン動作確認済み - Claude Code統合成功  

---

## 🏅 優先タスクリスト (実装優先度順)  <!-- 2025-06-25 -->
- [ ] **PoC: Pty.Net Linux/WSL 検証 (Throughput / SIGWINCH)**
- [ ] **PoC: IPC Unix Domain Socket フレーミング + 同時接続シリアライズ**
- [ ] **Worker Process 分離実装**
  - 各ペイン用独立プロセス起動
  - IPC経由でのI/Oリダイレクト
  - クラッシュ分離・再起動機構
- [ ] **プロセス間通信の安定化** (非同期ディスパッチ / メッセージロスト防止)
- [ ] **dev2/dev3 ペイン対応** (dev1パターン適用)
