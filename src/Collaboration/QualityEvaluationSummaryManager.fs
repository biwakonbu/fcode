...
type QualityEvaluationSummaryManager() =
    ...
    /// 品質評価サマリー生成
    member this.GenerateQualitySummary(metrics: QualityMetrics, testResults: string list, codeReviewFindings: string list) : QualityEvaluationSummary =
        ...
        // 改善提案生成
        let recommendations = this.GenerateRecommendations(metrics, testResults, codeReviewFindings)
    ...
    member private this.CalculateCodeQuality(metrics: QualityMetrics) : float =
        // 空データの場合は0を返す
        if metrics.CodeCoverage = 0.0 && metrics.CodeComplexity = 0.0 && metrics.SecurityVulnerabilities = 0 then
            0.0
        else
            ...
            // 複雑度スコア（複雑度が低いほど高スコア）
            let complexityScore = 
                if metrics.CodeComplexity <= 0.0 then 1.0
                elif metrics.CodeComplexity <= 2.5 then 0.95 // 高品質コード
                else Math.Max(0.0, 1.0 - (metrics.CodeComplexity - 1.0) / 10.0)
            
            // セキュリティスコア
            let securityScore = 
                if metrics.SecurityVulnerabilities = 0 then 1.0
                else Math.Max(0.0, 1.0 - float metrics.SecurityVulnerabilities / 10.0)
            
            coverageScore * coverageWeight + complexityScore * complexityWeight + securityScore * securityWeight
    
    /// 課題抽出
    member private this.ExtractIssues(codeReviewFindings: string list) : string list =
        codeReviewFindings
        |> List.filter (fun finding -> 
            finding.Contains("重大") || finding.Contains("中程度") || finding.Contains("軽微"))
        |> List.map (fun finding -> finding.Trim())
    
    /// 改善提案生成
    member private this.GenerateRecommendations(metrics: QualityMetrics, testResults: string list, codeReviewFindings: string list) : string list =
